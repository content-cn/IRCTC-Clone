...
const TrainSearchResults = () => {
  ...
  useEffect(() => {
    ...
    if (searchParams.travelClass) {
      filtered = filtered.filter(train => {
        ...
        return classDisplayName ? Object.keys(train.price).includes(classDisplayName) : true;
      });
    }
    
    // NEW CODE: Filter by travel class checkboxes
    const activeClassFilters = Object.keys(filters.travelClass).filter(key => filters.travelClass[key]);
    ...
    const activeTypeFilters = Object.keys(filters.trainType).filter(key => filters.trainType[key]);
    ...
    const activeTimeFilters = Object.keys(filters.departureTime).filter(key => filters.departureTime[key]);
  ...
  const displayTrains = (searchParams.from || searchParams.to || searchParams.travelClass || 
    Object.values(filters).some(category => Object.values(category).some(value => value))) 
    ? filteredTrains 
    : trains;
      ...
      <div className={styles.container}>
          ...
          <div className={styles.filterColumn}>
            ...
            <label className={styles.checkboxLabel}>
              <input 
                ...
                onChange={() => handleFilterChange('travelClass', 'AC First Class')}
              /> 
              AC First Class (1A)
            </label>
            <label className={styles.checkboxLabel}>
        ...
        <div className={styles.trainList}>
          {displayTrains.length === 0 ? (
            <div className={styles.noTrains}>No trains found for this route.</div>
          ) : (
