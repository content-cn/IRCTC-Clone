...
function BookingPage() {
  ...
  useEffect(() => {
    ...
    if (location.state?.trainNumber) {
      ...
      setTrainDetails({
        ...
        travelClass: location.state.travelClass || (classes.length > 0 ? classes[0] : 'AC Chair Car'),
  ...
  const handleSubmit =async (e) => {
    ...
    // Validate form data
    const isFormValid = passengers.every(p => p.name && p.age) && contactInfo.email && contactInfo.phone;
        ...
        <div className={styles.detailsGrid}>
          ...
          <div>
            ...
            <select 
              ...
              style={{ marginLeft: '10px', padding: '3px', borderRadius: '3px' }}
            >
              {availableClasses.map(cls => (
                <option key={cls} value={cls}>{cls} (₹{classPrice[cls]})</option>
              ))}
            </select>
          </div>
          <div>
